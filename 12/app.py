...
app.config['MYSQL_DB'] = 'user-system2'
...
def edit():
    ...
    if 'loggedin' in session:
        ...
        if request.method == 'POST' and 'name' in request.form and 'userid' in request.form and 'role' in request.form and 'country' in request.form and 'description' in request.form:
            userName = request.form['name']
            role = request.form['role']
            country = request.form['country']
            ...
            if 'avatar' in request.files:
                ...
                if file and allowed_file(file.filename):
                    ...
                    cursor.execute(
                        'UPDATE user SET avatar =% s WHERE userid =% s', (filepath, userId))
                    mysql.connection.commit()

            if not re.match(r'[A-Za-z0-9]+', userName):
                msg = 'name must contain only characters and numbers !'
            else:
                cursor.execute('UPDATE user SET  name =% s, role =% s, country =% s, description =% s WHERE userid =% s', (
                    userName, role, country, description, (userId, ), ))
                mysql.connection.commit()
                msg = 'User updated !'
                return redirect(url_for('users'))
        elif request.method == 'POST':
